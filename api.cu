...
void sgm::calcDisparity(cv::Mat left, cv::Mat right, cv::Mat &disp) {
    assert(left.cols == right.cols && left.rows == right.rows);
    assert(left.type() == right.type() && left.channels() == right.channels());
    assert(left.cols % 4 == 0 && right.rows % 4 == 0);
    assert(left.channels() == 1 && right.channels() == 1);
